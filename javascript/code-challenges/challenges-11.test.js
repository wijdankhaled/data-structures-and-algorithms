...
function transformToLis(obj){
  ...
  let newarr=Object.entries(obj).map(val=>{
...
const count = (target, input) => {
  // Solution code here...
  let count=input.reduce((x,y)=> x+y.reduce((x,y)=> y=== target ?x+1 :x, 0),0);
  return count;
};
...
const totalSum = (input) => {
  // Solution code here...
let sumation=input.reduce((a,b)=>a+b.reduce((a,b)=>a+b ,0),0);
return sumation;
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  let newarr=input.map(val=>val.filter(val=>!(val%5)));
  let checkimgString=newarr.map(val=>val.filter(val=>typeof(val)!=='string'));
  let result=checkimgString.map(val=>val.map(val=>Math.pow(2,val)));
  return result;
};
...
let findMaleAndFemale = (data) => {
  // Solution code here...
  let newarr=data.filter(val=>val.gender==='female' || val.gender=== 'male')
